...
!(function ($, window, document, undefined) {
  ...
  $(document).ready(function () {
    ...
    $('table').each(function () {
      if ($(this).data('table') == 'dnd') {
        $(this).tableDnD({
          onDragStyle:
            ($(this).data('ondragstyle') && parseStyle($(this).data('ondragstyle'))) || null,
          onDropStyle:
            ($(this).data('ondropstyle') && parseStyle($(this).data('ondropstyle'))) || null,
          onDragClass:
          ...
          onDragStop:
            $(this).data('ondragstop') && new Function('table', 'row', $(this).data('ondragstop')),
          ...
          serializeRegexp:
            ($(this).data('serializeregexp') && new RegExp($(this).data('serializeregexp'))) ||
            /[^\-]*$/,
  ...
  jQuery.tableDnD = {
    ...
    makeIndented(table) {
      ...
      config.hierarchyLevel &&
        $(rows).each(function () {
          ...
          (indentLevel <= config.hierarchyLevel && $(this).data('level', indentLevel)) ||
            $(this).data('level', 0);
    ...
    initialiseDrag(dragObject, table, target, e, config) {
    ...
    autoScroll(mousePos) {
      ...
      (mousePos.y - yOffset < config.scrollAmount && window.scrollBy(0, -config.scrollAmount)) ||
        (windowHeight - (mousePos.y - yOffset) < config.scrollAmount &&
          window.scrollBy(0, config.scrollAmount));
    },
    moveVerticle(moving, currentRow) {
      if (
        moving.vertical != 0 &&
    ...
    moveHorizontal(moving, currentRow) {
      const config = this.currentTable.tableDnDConfig;
      let currentLevel;
    ...
    findDragDirection(x, y) {
      const { sensitivity } = this.currentTable.tableDnDConfig;
      const { oldX } = this;
      const { oldY } = this;
    ...
    processMouseup() {
      ...
      $(document).unbind(moveEvent, this.mousemove).unbind(endEvent, this.mouseup);
    ...
    tableData(table) {
      ...
      if (!table || !table.id || !table.rows || !table.rows.length)
        return {
          error: { code: 500, message: 'Not a valid table, no serializable unique id provided.' },
        };

      rows = (config.autoCleanRelations && table.rows) || $.makeArray(table.rows);
      ...
      data[currentID] = [];
      !config.autoCleanRelations && $(rows[0]).data('level') && rows.unshift({ id: 'undefined' });
